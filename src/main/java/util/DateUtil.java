...
public class DateUtil {
	...
	public static List<java.sql.Date> getSqlDatesByYMD(String ymdFrom, String ymdTo) {
		List<java.sql.Date> dateList = new ArrayList<java.sql.Date>();
		try {
			Date dateFrom = DateUtil.parseSQLDate(ymdFrom, null);
			Date dateTo = DateUtil.parseSQLDate(ymdTo, null);
			Date tmpDate = new Date(dateFrom.getTime());
			while (tmpDate.compareTo(dateTo) <= 0) {
				dateList.add(new java.sql.Date(tmpDate.getTime()));
				tmpDate = new Date(DateUtils.addDays(tmpDate, 1).getTime());
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
